...
class Info extends React.Component {
  ...
  handleClick = action => {
    this.props.history.push(
      action === "next" ? "/lets-talk-react/skills" : "/lets-talk-react"
    );
  };
  ...
  validate = (name, email, verified, phone, city, country) => {
    ...
    const errors = {
      name: regexName.test(name)
        ? ""
        : "Please enter full name with alphabetic characters",
  ...
  render() {
    ...
    const top_left = [
      ...
      { name: "verified", placeholder: "Re-enter email" }
    ];
    const top_right = [{ name: "phone", placeholder: "Phone*" }];
    const low_middle_top = [
      { name: "city", placeholder: "City*" },
      { name: "state", placeholder: "State" },
      { name: "country", placeholder: "Country/Region*" },
      { name: "post", placeholder: "Zip/Postal code" }
    ];
      ...
      <form onSubmit={this.handleSubmit}>
        ...
        <div className="middle-top">
          ...
          <input
            ...
            value={info.more}
            onChange={e => this.handleChange(e, "more")}
          />
        </div>
